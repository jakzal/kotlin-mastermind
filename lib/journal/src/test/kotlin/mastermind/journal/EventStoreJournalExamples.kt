...
class EventStoreJournalExamples {
    ...
    fun `it executes the handler on an existing stream`() = runTest {
        ...
        journal.stream(streamName) {
            this shouldBe loadedStream(streamName, Event1("A1"))
            append(Event1("B1"))
        } shouldSucceedWith loadedStream(streamName, Event1("A1"), Event1("B1"))
    }
    ...
    fun `it returns the eventstore failure if the event store fails to append events`() = runTest {
        ...
        val journal = with(eventStore) { EventStoreJournal() }
